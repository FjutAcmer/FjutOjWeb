<template>
    <div class="docker">
        <el-tabs v-model="tableTabValue" @tab-click="handleClick" type="border-card" tab-position="top" class="tab">
            ...
            <el-tab-pane label="题目描述" name="2">
                ...
                <div style="margin:2% 5% 2% 5%">
                    <div>
                        ...
                        <el-tag type="success" v-if="isAc"><i class="el-icon-check"  style="color:green;margin-right:10px"></i>已通过</el-tag>
                        <el-tag type="danger" v-if="!isAc"><i class="el-icon-close"  style="color:red;margin-right:10px"></i>未通过</el-tag>
                    ...
                    <div style="width:100%;display:inline-flex">
                        <div class="card-div">
                            <el-card class="list-div">
                                <div class="clearfix">
                                    <span style="float:left;"><h2>Description</h2></span>
                                </div>
                                <div class="text item">
                                    <span style="float:left"><div style="text-align:left;font-size:20px;margin-bottom:10px;" v-html="maindata.dis?maindata.dis:'null'"></div></span>
                                </div>
                            </el-card>
                            <el-card class="list-div">
                                <div class="clearfix">
                                    <span style="float:left;"><h2>Input</h2></span>
                                </div>
                                <div class="text item">
                                    <span style="float:left"><div style="text-align:left;font-size:20px;margin-bottom:10px;" v-html="maindata.input?maindata.input:'null'"></div></span>
                                </div>
                            </el-card>
                            <el-card class="list-div">
                                <div class="clearfix">
                                ...
                                <div class="text item">
                                    <span style="float:left"><div style="text-align:left;font-size:20px;margin-bottom:10px;" v-html="maindata.output?maindata.output:'null'"></div></span>
                                </div>
                            </el-card>
                            <el-card class="list-div">
                                <div class="clearfix">
                            ...
                            <el-card class="list-div">
                                ...
                                <div class="text item">
                                    <span style="float:left">
                                        <div style="text-align:left;font-size:20px;margin-bottom:10px;padding-right:10px" v-html="simpledata?simpledata.output:'没有例子'"></div>
                                    </span>
                                </div>
                            </el-card>
                            <div class="submitDiv">
                                <div style="display:inline-flex;float:left;width:100%">
                                    <div style="width:10%;margin-right:2%"><p>language</p></div>
            ...
            <el-tab-pane label="在线测试" name="3">
                <el-card :body-style="{ padding: '0px' }" class="box-card">
                    <el-pagination style="float:left" layout="prev, pager, next" :current-page="currentPage" @current-change="getContestStatu" :total="currentTotal*10"></el-pagination>
                    <el-table style="width:100%;" :data="this.tableData">
                            ...
                            <el-table-column label="评测结果" style="width:20%">
                                <template slot-scope="scope">
                                    <el-tag size="medium" type="success" v-show="scope.row.result==1">{{ scope.row.otherinfo }}</el-tag>
                                    <el-tag size="medium" v-show="scope.row.result==0||scope.row.result==10||scope.row.result==12||scope.row.result==13">{{ scope.row.otherinfo }}</el-tag>
                                    <el-tag size="medium" @click="getInfo(scope.row)" type="danger" v-show="scope.row.result==2||scope.row.result==3||scope.row.result==4||scope.row.result==5
                                    ||scope.row.result==6||scope.row.result==7||scope.row.result==8||scope.row.result==11">{{ scope.row.otherinfo }}</el-tag>
                                    <el-tag size="medium" type="warning" v-show="scope.row.result==9">{{ scope.row.otherinfo }}</el-tag>
                                </template>
                            </el-table-column>
                            <el-table-column label="语言" style="width:5%">
                                <template slot-scope="scope"><div @click="toCodeView(scope.row)">{{scope.row.submitlanguage}}</div> </template>
                            </el-table-column>
...
export default {
    ...
    methods:{
        ...
        toCodeView(row){
            // console.log(row.id);
            let {href} = this.$router.resolve({ name: 'CodeView', query: { id: row.id ,ruser: row.ruser,problem: row.pid,languge: row.submitlanguage,jadgeStatu: row.otherinfo}});
